...
header_info <- read.xlsx("/Volumes/Samsung_T5/Immport_UH2/SDY33/Header_SDY33.xlsx")                            
colnames(header_info)[1] <- "name" 

mm <- merge(fcs_meta_Data, header_info, by="name")
final_tab1 <- mm[which(mm$Panel == 2),] #Treg
final_tab2 <- mm[which(mm$Panel == 3),] #Treg
final_tab <- rbind(final_tab1, final_tab2)
write.csv(final_tab,"/Volumes/Samsung_T5/Immport_UH2/SDY33/Treg/MetaData_Treg_SDY33.csv",row.names=TRUE)